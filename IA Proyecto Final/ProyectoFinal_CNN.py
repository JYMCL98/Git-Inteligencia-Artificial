...
data = pd.read_csv('D:/Documents/Chapingo/7° semestre/Inteligencia Artificial/IA-Proyecto Final/IA Proyecto Final/Chayotes.csv')
data2 = np.array(data)
m, n = data2.shape
...
for epoca in range(3):
    ...
    for i, (im, label) in enumerate(zip(train_images, train_labels)):
        if i % 100 == 0:
            print(
                '[Paso %d] de 1000| pérdida %.3f | Exactitud: %d%%' %
...
while True:
    ...
    escala_grises = cv.putText(frame,objt,(x_imagen,50),cv.FONT_HERSHEY_COMPLEX, 1.5,color,1)
