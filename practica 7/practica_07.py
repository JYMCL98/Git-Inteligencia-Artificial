...
# Verificacion del algoritmo de backpropagation
a_verificacion = np.array(np.zeros((1,Q)))
for q in range (Q):
	a_verificacion[:,q] = tanh(np.dot(W2,tanh(np.dot(W1,P[:,q].reshape(-1,1))+b1))+b2)

print("Valores esperados: ",T)
print("Valores de la NN: ", a_verificacion)
...
# Verificacion de NN Backpropagation
a_verificacion = np.array(np.zeros((1,Q)))
for q in range(Q):
	a_verificacion[:,q] = tanh(np.dot(W2,tanh(np.dot(W1,P[:,q].reshape(-1,1))+b1))+b2)
...
ax2.scatter(x2[100:],a_verificacion[:,100:].reshape(-1,1),c="blue")
ax2.scatter(x2[:100],a_verificacion[:,:100:].reshape(-1,1),c="red")
#plt.show()
...
ax3.scatter(P[0][100:],P[1][100:],marker='x',c='red')
ax3.scatter(P[0][:100],P[1][:100],marker='o',c='blue')
ax3.contour(u,v,z.T,2,linewidths = np.arange(-0.9,0,0.9))
